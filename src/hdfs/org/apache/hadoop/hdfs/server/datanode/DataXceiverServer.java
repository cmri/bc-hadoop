...
class DataXceiverServer implements Runnable, FSConstants {
  ...
  public void run() {
    while (datanode.shouldRun) {
      Socket s = null; //add by guoleitao, 20120517. HDFS-2286 & HDFS-2452
      try {
        //Socket s = ss.accept(); //del by guoleitao
    	s = ss.accept();  // add by guoleitao
      ...
      //add by guoleitao, 20120517 
        ...
        if (datanode.shouldRun) { // add by guoleitao
      ...
      //add by guoleitao, 20120517. HDFS-2286 & HDFS-2452
      } catch (OutOfMemoryError ie) {
        IOUtils.closeSocket(s);
        // DataNode can run out of memory if there is too many transfers.
        // Log the event, Sleep for 30 seconds, other transfers may complete by
        // then.
        LOG.warn("DataNode is out of memory. Will retry in 30 seconds (HDFS-2286&2452).", ie);
        try {
