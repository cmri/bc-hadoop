...
public class FSNamesystem implements FSConstants, FSNamesystemMBean,
  ...
  private NameNode nameNode = null; //add by guoleitao for nn ha
  ...
  private void initialize(NameNode nn, Configuration conf) throws IOException {
    ...
    this.nameNode = nn; //add by guoleitao for nn ha.
  ...
  class SafeModeInfo {
    ...
    private void reportStatus(String msg, boolean rightNow) {
      ...
      if(!rightNow && (curTime - lastStatusReport < 20 * 1000))
        return;
      if(!manualOverrideSafeMode){ //added by guoleitao
...
//        leaveSafeMode(false);//del by guoleitao
    	  //add by guoleitao
  ...
  /**
   * the following added by guoleitao for namenode ha, 20120819
   * */
	    ...
	    try {
	      ...
	      for (int i = 0; i < receivedAndDeletedBlocks.length; i++) {
	        ...
	        if (DFSUtil.isDeleted(receivedAndDeletedBlocks[i])) {
	          ...
	          blockReceived(node,receivedAndDeletedBlocks[i],
...
//	      writeUnlock();
	      ...
	      for (int i = 0; i < receivedAndDeletedBlocks.length; i++) {
	        ...
	        if (blk == null) 
	            continue;
	        ++count;
	        if (blk.getNumBytes() != Long.MAX_VALUE) {       
	    ...
	    try {
	      ...
	      for (int i = 0; i < receivedAndDeletedBlocks.length; i++) {
	        ...
	        if (DFSUtil.isDeleted(receivedAndDeletedBlocks[i].getBlock())) {
	          ...
	          blockReceived(node,receivedAndDeletedBlocks[i].getBlock(),
	              receivedAndDeletedBlocks[i].getDelHints());
	          received++;
	        }
	      }
