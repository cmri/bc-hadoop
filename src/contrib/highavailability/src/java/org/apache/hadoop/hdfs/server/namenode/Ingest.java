...
public class Ingest implements Runnable {
  ...
  private int loadFSEdits(File edits) throws IOException {
    ...
    int numOpGetDelegationToken = 0, 
        numOpRenewDelegationToken = 0, numOpCancelDelegationToken = 0,
        numOpUpdateMasterKey = 0; //add by guoleitao
    long highestGenStamp = -1;//add by guoleitao
...
//        if (supportChecksum) { //del by guoleitao
//          in = new DataInputStream(new CheckedInputStream(fp, checksum));
//        }
        reopen = false;
      }
...
//              FSNamesystem.LOG.debug("Ingest: Invalid opcode, reached end of edit log " +
...
//          FSEditLog.validateChecksum(supportChecksum, rawIn, checksum, numEdits);//del guoleitao
          ...
          numOpSetRepl++;
          path = FSImage.readString(in);
          short replication = FSEditLog.adjustReplication(readShort(in));
//          FSEditLog.validateChecksum(supportChecksum, rawIn, checksum, numEdits);//del guoleitao
          fsDir.unprotectedSetReplication(path, replication, null);
          fsDir.fsImage.getEditLog().logSetReplication(path, replication);
          break;
          } 
//del by guoleitao, CONCAT_DELETE not supported
...
//          HdfsFileStatus dinfo = fsDir.getHdfsFileInfo(d);
          HdfsFileStatus dinfo = fsDir.getFileInfo(d);//add by guoleitao
...
//          FSEditLog.validateChecksum(supportChecksum, rawIn, checksum, numEdits);
          ...
          if ((highestGenStamp != -1) && (highestGenStamp + 1 != lw)) {
            throw new IOException("Ingest: OP_SET_GENSTAMP tried to set a genstamp of " + lw + 
...
//          FSEditLog.validateChecksum(supportChecksum, rawIn, checksum, numEdits);
          ...
          //the following are added by guoleitao, 20121222
...
//          } // del by guoleitao, not check checksum
          numEdits++;
...
//          fsNamesys.writeUnlock();
      ...
      if (error || running) {
        ...
        catchingUp = (fc.size() - fc.position() > catchUpLag);
        in = rawIn = new DataInputStream(fp);
//        if (supportChecksum) {	//del by guoleitao, not support checksum
//          in = new DataInputStream(new CheckedInputStream(fp, checksum));
//        }
...
//  private static final byte OP_CONCAT_DELETE = 16; // concat files. //del by guoleitao, not support CONCAT_DELETE
  private static final byte OP_GET_DELEGATION_TOKEN = 15; //add by guoleitao
  private static final byte OP_RENEW_DELEGATION_TOKEN = 16; //add by guoleitao
  private static final byte OP_CANCEL_DELEGATION_TOKEN = 17; //add by guoleitao
  private static final byte OP_UPDATE_MASTER_KEY = 18; //add by guoleitao
}
