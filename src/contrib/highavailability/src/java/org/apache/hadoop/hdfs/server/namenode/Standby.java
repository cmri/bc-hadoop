...
//      fsnamesys.saveNamespace(false, false);//del by guoleitao
      fsnamesys.saveNamespace();//add by guoleitao
    } catch (IOException ex) {
...
//      primaryNamenode.rollFsImage(new CheckpointSignature(fsImage)); //del by guoleitao
      primaryNamenode.rollFsImage();//add by guoleitao
      checkpointInProgress = false;
  ...
  void initSecondary(Configuration conf) throws IOException {
    ...
    LOG.info("--7--- in Standby: fsName = "+fsName);
