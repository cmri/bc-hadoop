...
//import org.apache.hadoop.security.UnixUserGroupInformation; //del
import org.apache.hadoop.security.UserGroupInformation; //add
...
//  private UnixUserGroupInformation ugi; //del
  private UserGroupInformation ugi; //add
...
//      UnixUserGroupInformation ugi) throws IOException {
  ...
  public int run(String argv[]) throws Exception {
    ...
    //not support services in cdh3u4, del, guoleitao
...
//        AvatarNode.initializeGenericKeys(originalConf, service);
        ...
        if ("-updateZK".equals(zkCmd)) {
          ...
          getZooKeeper();
            ...
            exitCode = -1;
            System.err.println(zkCmd.substring(1) + ": Unknown ZK command");
            printUsage("");
        }
//      }
...
//    InetSocketAddress defaultAddr = NameNode.getClientProtocolAddress(originalConf); //del
    InetSocketAddress defaultAddr = NameNode.getAddress(originalConf); //add
...
//    defaultAddr = NameNode.getClientProtocolAddress(originalConf);	//del
    defaultAddr = NameNode.getAddress(originalConf);	//add
...
//    InetSocketAddress addr = NameNode.getClientProtocolAddress(conf);	//del
    InetSocketAddress addr = NameNode.getServiceAddress(conf,true);	//add
    if (addr == null) {
      System.out.println( "fs.default.name for primary service is not defined");
      return;
    }
//    defaultAddr = NameNode.getClientProtocolAddress(originalConf); //del
    defaultAddr = NameNode.getServiceAddress(originalConf,true); //add
    if (defaultAddr == null) {
      System.out.println( "fs.default.name for default service is not defined");
      return;
    }    
